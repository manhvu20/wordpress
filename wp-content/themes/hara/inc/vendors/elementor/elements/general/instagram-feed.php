...
if (! defined('ABSPATH') || function_exists('Hara_Elementor_Instagram_Feed')) {
 ...
 * @since 1.0.0
 */
class Hara_Elementor_Instagram_Feed extends Hara_Elementor_Carousel_Base
{
    /**
     * Get widget name.
     *
    ...
    {
        return esc_html__('Hara Instagram Feed', 'hara');
    }
    ...
    protected function register_controls()
    {
        ...
        $this->add_control(
            'heading_settings',
            [
                'label' => esc_html__('Settings', 'hara'),
        ...
        $this->add_control(
            'select_feeds',
            [
                'label'     => esc_html__('Select Feeds', 'hara'),
                'type'      => Controls_Manager::SELECT,
        ...
        $this->add_control(
            ...
            [
                ...
                'raw'             => sprintf(__('Please settings each feed <a href="%s" target="_blank">here</a>', 'hara'), admin_url('?page=sbi-feed-builder')),
            ]
        );
    ...
    protected function register_controls_load_more()
    {
        $this->start_controls_section(
            'section_load_more',
            [
                'label' => esc_html__('Load More', 'hara'),
    ...
    protected function register_controls_item_style()
    {
        ...
        $this->add_responsive_control(
            'item_spacing',
            [
                'label'     => esc_html__('Space Between', 'hara'),
    ...
    protected function get_select_feeds() {
        ...
        foreach($exported_feeds as $feed_id => $feed) {
            $feeds[$feed['id']] = esc_html__('Feed', 'hara').' '.$feed['id'];
        }

        return $feeds;
    }
}
$widgets_manager->register(new Hara_Elementor_Instagram_Feed());