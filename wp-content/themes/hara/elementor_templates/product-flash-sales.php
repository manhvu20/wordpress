...
$this->add_render_attribute('wrapper', 'class', [ $product_style, $this->get_name_template() .'-'. $position_displayed, $this->deal_end_class() ]);
?>
    ...
    <?php
        if (!empty($heading_title) || !empty($heading_subtitle) || (isset($end_date) && !empty($end_date))) {
            ...
            if (isset($end_date) && !empty($end_date)) {
                hara_tbay_countdown_flash_sale($end_date, $date_title, $date_title_ended, true);
            } ?>
            </div>
            <?php
        }
